...
export default function TourDetailPage() {
  ...
  useEffect(() => {
    const loadTour = async () => {
      try {
        ...
        setMessage(error.response?.data?.message || "Khong tai duoc chi tiet tour.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleBookTour = async () => {
    ...
    if (!tour?.id) {
      setMessage("Khong tim thay tour hop le de dat.");
      return;
    }
    ...
    try {
      ...
      setMessage(error.response?.data?.message || "Dat tour that bai.");
    } finally {
  ...
  if (loading) {
    return <section className="page-section">Dang tai chi tiet tour...</section>;
  }

  if (!tour) {
    return <section className="page-section">{message || "Khong tim thay tour."}</section>;
  }
    ...
    <section className="page-section detail-layout">
      ...
      <div className="detail-card">
        ...
        <button type="button" className="primary-button" onClick={handleBookTour} disabled={bookingLoading}>
          {bookingLoading ? "Dang dat tour..." : "Dat tour"}
        </button>
        {message && <p className="message error">{message}</p>}
        {bookingResult && (
          <div className={`message ${bookingResult.booking.status === "CONFIRMED" ? "success" : "warning"}`}>
            <p>{bookingResult.message}</p>
            <p>Ma booking: {bookingResult.booking.id}</p>
            <p>Thanh toan: {bookingResult.payment.status}</p>
          </div>
        )}
      </div>
    </section>
